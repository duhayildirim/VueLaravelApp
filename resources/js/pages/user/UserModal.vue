                ...
                <div class="modal-body">
                    ...
                    <form @submit.prevent="true">
                        <div class="form-group row">
                            <label class="col-sm-3 col-form-label">Kulllanıcı Adı </label>
                            <div class="col-sm-9">
                                <input v-model="item.name" class="form-control" required>
                            </div>
                        </div>
                        <div class="form-group row">
                            <label class="col-sm-3 col-form-label">Sifre </label>
                            <div class="col-sm-9">
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-sm-9">
                                <input type="email" v-model="item.email" class="form-control" required>
                            </div>
                        </div>
                    </form>
                </div>
...
<script>
    export default {
        ...
        methods: {
            saveItem() {
                if(this.item.id > 0) {
                    window.axios.put('/users/'+ this.item.id , this.item)
                        .then(response => {
                    ...
                    window.axios.post('/users' , this.item)
                        .then(response => {
                            if(response.data.success) {
                                this.$emit('onSaved' , this.item);
                                $('#userModal').modal('hide');
